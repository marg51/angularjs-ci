  ...
  req = updateStatus {status: 'pending', sha: data.after, message: 'Wait for the tests'}, (res) ->
    ...
    res.on 'end', ->
      ...
      if result.id?
        ...
        tests = spawn('./post-update.sh',[ref.split('/').pop(),data.after.slice(0,9)])
...
updateStatus = (params, fn) ->
  ...
  req.write(JSON.stringify({  "state": params.status,  "target_url": host_build+"/build/"+params.sha.slice(0,9)+'.html',  "description": params.message || "no infos",  "context": "continuous-integration/angularjs-ci"}));
